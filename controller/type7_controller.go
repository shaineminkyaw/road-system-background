...
type type7Controller struct {
	H *Handler
}

func NewType7Controller(h *Handler) *type7Controller {
	return &type7Controller{
		H: h,
	}
}

func (ctr *type7Controller) Register() {
	h := ctr.H
	group := h.R.Group("/api/type7_table/", middleware.Cors(), middleware.AuthMiddleware())
	group.GET("list", ctr.list)
	group.POST("create", middleware.Authorize(h.Enforcer, "/api/type7_table/create", "POST"), ctr.create)
	group.POST("update", middleware.Authorize(h.Enforcer, "/api/type7_table/update", "POST"), ctr.edit)
	group.POST("delete", middleware.Authorize(h.Enforcer, "/api/type7_table/delete", "POST"), ctr.erase)

}

//type7 req
type ReqTableType7List struct {
...
//type7 response
type RespTableType7List struct {
...
//type7 list
func (ctr *type7Controller) list(c *gin.Context) {
	//
	resp := &dto.RespObj{}
	req := ReqTableType7List{}
...
//type7 create request
type ReqType7Create struct {
	RoadType    int8   `json:"road_type" form:"road_type" binding:"required"`
...
// type7 create
func (ctr *type7Controller) create(c *gin.Context) {
	//
...
//type7 edit
func (ctr *type7Controller) edit(c *gin.Context) {
	//
...
//delete request for type7
type ReqDeleteType7 struct {
	RoadType uint64 `json:"road_type"`
	ID       uint64 `json:"tid"`
}

//type7 delete
func (ctr *type7Controller) erase(c *gin.Context) {
	//
