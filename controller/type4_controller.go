...
type type4Controller struct {
	H *Handler
}

func NewType4Controller(h *Handler) *type4Controller {
	return &type4Controller{
		H: h,
	}
}

func (ctr *type4Controller) Register() {
	h := ctr.H
	group := h.R.Group("/api/type4_table/", middleware.Cors(), middleware.AuthMiddleware())
	group.GET("list", ctr.list)
	group.POST("create", middleware.Authorize(h.Enforcer, "/api/type4_table/create", "POST"), ctr.create)
	group.POST("update", middleware.Authorize(h.Enforcer, "/api/type4_table/update", "POST"), ctr.edit)
	group.POST("delete", middleware.Authorize(h.Enforcer, "/api/type4_table/delete", "POST"), ctr.erase)

}

//type4 req
type ReqTableType4List struct {
...
//type4 response
type RespTableType4List struct {
...
//type4 list
func (ctr *type4Controller) list(c *gin.Context) {
	//
	resp := &dto.RespObj{}
	req := ReqTableType4List{}
	...
	betResp := make([]*RespTableType4List, 0)
	...
	for _, bet := range bets {
		betResp = append(betResp, &RespTableType4List{
...
//type4 create request
type ReqType4Create struct {
	RoadType    int8   `json:"road_type" form:"road_type" binding:"required"`
...
// type4 create
func (ctr *type4Controller) create(c *gin.Context) {
	//
...
//type4 edit
func (ctr *type4Controller) edit(c *gin.Context) {
	//
...
//delete request for type4
type ReqDeleteType4 struct {
	RoadType uint64 `json:"road_type"`
	ID       uint64 `json:"tid"`
}

//type4 delete
func (ctr *type4Controller) erase(c *gin.Context) {
	//
